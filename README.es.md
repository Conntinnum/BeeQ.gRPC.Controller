...
## Auto Documentación
La librería viene con autodocumentador que puede ser accedido una ves que inició la aplicación y devolverá información de los servicios disponibles.
...
## Llamadas Grpc Externas
Aunque el objetivo de la librería es abstraer al programador del funcionamiento de Grpc y poder utilizar directamente los objetos class de C#. \
Existe la posibilidad de hacer llamadas Grpc a estos servicios desde una app externa o que no utilice esta librería. \
El proto es el siguiente:
``` proto
syntax = "proto3";
...
```
Se debe invocar al método Execute completando los parámetros, donde: `service` es el nombre del servicio Grpc (equivale al nombre que se establece en `[GrpcService(NOMBRE)]`, mientras que `method` y `version` son los establecidos en `[GrpcMethod(METHOD, VERSION)]`. \
El Payload es el objeto que viaja en el request en forma binaria y dependerá del objeto que se esté enviando. \
La presente librería utiliza el standard de `MessagePack` para las conversiones de los objetos a `bytes` y viceversa.

## Minimal API's
La librería ahora permite la compatibilidad con Minimal API. \
Al no haber Controllers, se debe establecer el nombre del servicio por cada mapeo REST.
...
app.Run();
```
Es importante que `UseGrpcController` sea invocado luego de las definiciones de `Map`.
